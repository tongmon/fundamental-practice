...
1. 팩터리 메서드나 팩터리는 클래스의 생성자 대신에 객체를 생성해주는 역할을 한다.  

2. 클래스의 생성자를 사용하는 것보다 팩터리 패턴을 사용하는 것이 이름 짓기가 가능해 가독성이 더 좋다.  

3. 팩터리 패턴은 nullptr 등을 반환하는 형식으로 객체 생성을 거부할 수 있지만 클래스 생성자는 익셉션을 일으키는 것 밖에 못한다.  

4. 팩터리는 클래스 생성자보다 캐싱과 메모리 최적화를 구현하기 편하다.  

5. 빌더 패턴은 하나의 객체를 여러 단계를 거쳐 생성하지만 팩터리 패턴은 객체를 한 번에 생성하는 것이 일반적이다.  